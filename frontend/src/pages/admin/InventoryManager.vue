  ...
  <div>
    ...
    <div class="grid grid-cols-3 gap-4 mb-6">
        <el-card shadow="hover" class="border-l-4 border-red-500">
            ...
            <div class="text-2xl font-bold text-red-600 mt-1">{{ lowStockCount }} <span class="text-sm text-gray-400">cuốn</span></div>
        </el-card>
        <el-card shadow="hover" class="border-l-4 border-green-500">
            ...
            <div class="text-2xl font-bold text-green-600 mt-1">{{ safeStockCount }} <span class="text-sm text-gray-400">cuốn</span></div>
        </el-card>
    ...
    <el-card shadow="never" class="rounded-lg border-none">
      <el-table :data="books" style="width: 100%" v-loading="loading" stripe border height="500">
    ...
    <el-dialog v-model="dialogVisible" title="Phiếu Nhập Hàng" width="400px">
      <div v-if="selectedBook" class="text-center mb-4">
          ...
          <div class="text-gray-500">Tồn kho hiện tại: <span class="font-bold text-black">{{ selectedBook.stock_quantity }}</span></div>
      </div>
...
import { Box, Refresh, Plus } from '@element-plus/icons-vue';
...
// --- TÍNH TOÁN THỐNG KÊ ---
const lowStockCount = computed(() => books.value.filter(b => b.stock_quantity < 10).length);
const safeStockCount = computed(() => books.value.filter(b => b.stock_quantity >= 10).length);
const totalValue = computed(() => books.value.reduce((sum, b) => sum + (b.price * b.stock_quantity), 0));

const formatCurrency = (val) => new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(val);
const getImageUrl = (book) => (book.BookImages && book.BookImages.length > 0) ? book.BookImages[0].book_image_url : 'https://placehold.co/100x150';
...
const fetchData = async () => {
  ...
  } catch (error) { ElMessage.error('Lỗi tải dữ liệu kho'); } 
  finally { loading.value = false; }
};

const openImportDialog = (book) => {
    selectedBook.value = book;
    importQuantity.value = 10; // Mặc định nhập 10 cuốn
...
const handleImport = async () => {
    ...
    try {
        ...
        fetchData(); // Load lại để thấy số mới
    } catch (error) {
