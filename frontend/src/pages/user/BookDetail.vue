...
// Lấy ảnh đầu tiên của sách để hiển thị
...
const fetchBookDetail = async (id) => {
  ...
  try {
    // Gọi API Backend lấy sách theo ID
...
// Quan trọng: Watch ID trên URL để load lại sách khi người dùng bấm vào phần "Gợi ý"
...
const handleAddToCart = async () => {
  if (book.value) {
    // Chuẩn bị dữ liệu gửi sang store
    const productToAdd = {
    ...
    // Gọi hàm addToCart trong store (đã viết ở trên)
